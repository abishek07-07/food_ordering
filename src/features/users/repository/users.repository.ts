...
export class UsersRepository {
  ...
  async findByEmail(email: string): Promise<Partial<Users> | null | undefined> {
    return this.db<Users>("users")
      .select("email",
        ...
        "middle_name",
        "password",
        "slug").where("email", email)
      .first();

  }


  async findUserBySlug(slug: string): Promise<Partial<IUserExceptPassword> |  undefined> {
  ...
  async updateUserBySlug(slug: string, patch: Partial<InsertUserRequst>): Promise<number> {
